...
def get_terms():
    ...
    '''
    d = get_data('/Curriculum/SOC/v1/Terms')['getSOCTermsResponse']['Term']
    l = list((d,))
    return l
def get_schools(code):
    url = '/Curriculum/SOC/v1/Terms/'+str(code)+ '/Schools'
    return get_data(url)['getSOCSchoolsResponse']['School']

def get_subjects(code, schoolCode):
    url = '/Curriculum/SOC/v1/Terms/' + str(code) + '/Schools/' + str(schoolCode) + '/Subjects'
    return get_data(url)['getSOCSubjectsResponse']['Subject']

def get_catalognbrs(code, schoolCode, subjectCode):
    url = '/Curriculum/SOC/v1/Terms/' + str(code) + '/Schools/' + str(schoolCode) + '/Subjects/' + str(subjectCode) + '/CatalogNbrs'
    return get_data(url)['getSOCCtlgNbrsResponse']['ClassOffered']

def get_sections(code, schoolCode, subjectCode, catalogNbrs):
    url = '/Curriculum/SOC/v1/Terms/' + str(code) + '/Schools/' + str(schoolCode) + '/Subjects/' + str(subjectCode) + '/CatalogNbrs/' + str(catalogNbrs) + '/Sections'
    d = get_data(url)['getSOCSectionsResponse']['Section']
    if type(d) is list:
        return d
    else:
        l = list((d, ))
